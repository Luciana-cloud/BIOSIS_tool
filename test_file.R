...
scale = c("landscape")
# options are local, landscape and regional

## B.2) Who are you? (user)
...
if(scale == "local"){
  ...
  sieve$App.1 <- sieve$Landscape_scale
} else if (scale == "regional"){
  sieve$App.1 <- sieve$Regional_scale
}

if(user == "land_manager"){
  sieve$App.2 <- sieve$Land_Manager_persp
} else if (user == "business"){
  sieve$App.2 <- sieve$Business_persp
} else if (user == "policy_maker"){
  sieve$App.2 <- sieve$Policy_makers_persp
